...
public class RefreshAccessTokenCommandTests : BaseIntegrationTest
{
    ...
    {
        TokensGenerator tokensGenerator = ServiceProvider.GetRequiredService<TokensGenerator>();
        string refreshTokenString = tokensGenerator.GenerateTokens(user).RefreshToken;

        using (var context = CreateDbContext())
        {
            User userFromDb = context.Users.Single(u =>
                u.Login != null && u.Login.Value == "yegor"
            );
